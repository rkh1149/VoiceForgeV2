...
export async function DELETE(
  ...
  // 3. VoiceForge records (order respects foreign keys). The Neon HTTP
  // driver used by Drizzle here does not support interactive transactions.
  const runs = await db
    .select({ id: buildRuns.id })
    .from(buildRuns)
  ...
  await db.delete(appJobRuns).where(eq(appJobRuns.appId, appId));
  await db.delete(appScheduledJobs).where(eq(appScheduledJobs.appId, appId));
  await db
    .delete(appNotificationPreferences)
    .where(eq(appNotificationPreferences.appId, appId));
  await db.delete(appNotifications).where(eq(appNotifications.appId, appId));
  await db.delete(appFiles).where(eq(appFiles.appId, appId));
  await db
