...
- You may ONLY mutate approved generated-app files via write_file, patch_file, delete_file, or rename_file: src/app/, src/components/, src/lib/, and e2e/generated/. package.json, configs, src/app/globals.css, src/lib/template.test.ts, e2e/smoke.spec.ts, e2e/voiceforge-acceptance.ts, e2e/voiceforge-progress-reporter.ts, and all API routes are locked.
...
- STAGE 14D BROWSER ACCEPTANCE CONTRACT: For every supplied workflow acceptance journey, write a real Playwright journey under e2e/generated/. Import workflowJourneyTitle, workflowStepTitle, workflowSaveTitle, workflowHandoffTitle, voiceForgeRoleHeaders, acceptanceRunSuffix, expectPersistedAfterReload, expectDownloadedFile, and tinyPngUpload as needed from ../voiceforge-acceptance. Use workflowJourneyTitle in the test title and wrap every ordered contract step in test.step(workflowStepTitle(workflowId, contractStepId, description), ...). Exercise the visible route/control path with getByRole/getByLabel, not direct platform API calls, localStorage writes, hidden URLs, or placeholder assertions. A deliberate action/input step performs the user gesture; its following automatic/result steps assert the observable effects without looking for additional controls. Every navigate trace block must itself click/goto and assert the destination; every input trace block must itself fill/select/check and assert the entered or selected result. If one action also proves a handoff, nest the workflowStepTitle block inside the workflowHandoffTitle block so both markers own the action; never put the action in one trace block and only an assertion in the other. Prefer the app's concise visible command labels and prove the promised postconditions, rather than adding artificial controls or sentence-length aria-labels for internal effects. Keep each synthesized journey as its own focused test. Each Playwright test receives a fresh browser context: test.describe.serial controls order but does NOT preserve page state, localStorage, or sessionStorage between tests. When a journey declares dependsOnJourneyIds, replay the necessary prerequisite workflows through visible UI controls inside that dependent test (or a UI-only setup helper called by it); never depend on records, page state, or mutable module variables from an earlier test. Build record fixture names and emails with acceptanceRunSuffix() so a Playwright retry cannot create duplicate rows with the same accessible name. Before a state-clearing action such as Mark as watched, Delete, Remove, Archive, Complete, or Finish, capture any selected record title/id/date needed by later assertions; after the action, assert the success message or new state instead of trying to read the result the app correctly cleared. For downstream navigation, inspect the rendered app and use a stable visible navigation link when an optional contextual link disappears after the mutation. For every save, add test.step(workflowSaveTitle(saveId), ...) with page.reload() or expectPersistedAfterReload and assert the unique saved fixture remains visible. When one click creates several entity records, every supplied saveId still needs its own reload and visible fixture assertion; do not let one save marker borrow another save marker's reload. For every handoff, add test.step(workflowHandoffTitle(handoffId), ...), navigate through visible UI to the consumer route, and assert the producer's saved value is present in the named consumer control/result. After clicking a navigation link, wait for the destination URL or a destination-specific heading before calling page.reload(); never click a Next.js link and immediately reload because the reload can win the client-side navigation race and refresh the old route. Use getByLabel for native date/datetime inputs; they do not reliably expose the textbox role. Native select option elements are hidden unless the select is open: prove an option is loaded with expect(combobox).toContainText(label) or expect(combobox.getByRole("option", {name:label})).toHaveCount(1), never toBeVisible(). Use page.waitForEvent("download") for exports, setInputFiles(tinyPngUpload()) for uploads, browser contexts with voiceForgeRoleHeaders(role) for role checks, and granted geolocation with fixed coordinates for GPS. Helper functions may reduce repeated locator setup, but each trace block must call the helper that performs its specific action and assertion; never add a marker around an empty or unrelated block. Do not use request/API fixtures to manufacture the record whose UI workflow is being tested. Do not use test.skip, test.fixme, waitForTimeout, or assertions that only prove a page/heading exists.
...
Generate executable Playwright coverage for every journey and every ordered step above. Exact journey, workflow, step, control, save, and handoff ids are traceability keys: pass journey/step/save/handoff ids to the locked title helpers exactly and use vfControl(page, workflowId, controlId) for every step that declares a controlId. Scope repeated actions with vfRecord or vfRecords plus vfRecordControl. Assert the friendly accessible name separately with expectContractControl when useful; do not return to wording-based identity. Keep bounded journeys as separate focused tests. A journey with dependsOnJourneyIds must recreate the records it needs through visible UI actions in that test's fresh browser context; serial ordering alone does not preserve browser state between Playwright tests. Capture transient result identity before a completion/delete/status action clears it, and use the app's stable visible navigation for downstream screens. A generated spec is incomplete if it only mentions an id without performing the associated UI action and assertion.`;
}
