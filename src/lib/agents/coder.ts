...
- For platform integrations, import from src/lib/platform-integrations.ts. Call listPlatformIntegrationProviders and invokePlatformIntegration only with approved provider/action keys from the architecture. Stage 12A approved provider: providerKey "demo_directory" with actions "list_contacts", "lookup_contact", and "record_contact_note". Never call external URLs directly, never request or store API keys/tokens/webhook URLs in generated app state, and never create credential-management UI inside generated apps. Hide editor-only integration actions when session.canWrite is false and owner-only integration actions when session.canManage is false.
...
- "Please sign in" renders with no working sign-in button/link, disabled sign-in forever, or returns to the sign-in button immediately after authorization: load getPlatformSession first, store session.loginUrl, stop fetching records/files while signed_out/no_access, render PlatformSignInGate or a visible button that calls signInToPlatform(session) from every route, and redirect/render the dashboard when session.status is signed_in on the sign-in route.
