...
- For platform integrations, import from src/lib/platform-integrations.ts. Call listPlatformIntegrationProviders and invokePlatformIntegration only with approved provider/action keys from the architecture. Approved providers/actions: providerKey "demo_directory" with actions "list_contacts", "lookup_contact", and "record_contact_note"; providerKey "google_maps" with server-only actions "search_places", "get_place_details", "geocode_address", and "compute_route". For Google Maps trip-planning apps, render place/route results as lists, cards, itinerary timelines, tables, and saved records; do not embed Google map tiles or load Maps JavaScript yet. Never call external URLs directly, never request or store API keys/tokens/webhook URLs in generated app state, and never create credential-management UI inside generated apps. Hide editor-only integration actions when session.canWrite is false and owner-only integration actions when session.canManage is false.
