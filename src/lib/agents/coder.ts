...
- For platform search/reports over shared platform records, prefer the Stage 12B helpers from src/lib/platform-data.ts: searchPlatformRecords, listPlatformRecordSearchConfigs, listPlatformSavedFilters, savePlatformRecordFilter, deletePlatformSavedFilter, runPlatformRecordReport, and exportPlatformRecordsCsv. Use these for server-side search/filter/sort, saved filters, report rows, and CSV export instead of fetching every record and doing all work client-side. Saved-filter write controls must be hidden when session.canWrite is false; delete saved filters only when session.canManage is true.
...
- For platform integrations, import from src/lib/platform-integrations.ts. Call listPlatformIntegrationProviders and invokePlatformIntegration only with approved provider/action keys from the architecture. Approved providers/actions: providerKey "demo_directory" with actions "list_contacts", "lookup_contact", and "record_contact_note"; providerKey "google_maps" with actions "search_places", "get_place_details", "geocode_address", and "compute_route". For Google Maps trip-planning apps, render place/route results as lists, cards, itinerary timelines, tables, saved records, and an interactive map by importing GoogleMapsTripMap from src/components/voiceforge-google-map.tsx. Pass it place results and route output from the locked google_maps integration. For bicycle trip planners, call compute_route with travelMode:"BICYCLE", do not pass routingPreference, keep returned route warnings/safetyNotice visible, and pass the route to GoogleMapsTripMap so the locked component can show the bicycle layer. Do not write Google Maps script tags, direct Maps JavaScript calls, API key handling, external map URLs, or custom credential UI; the locked map component loads real tiles/pins/routes and falls back safely when browser maps are unavailable. Never call external URLs directly, never request or store API keys/tokens/webhook URLs in generated app state, and never create credential-management UI inside generated apps. Hide editor-only integration actions when session.canWrite is false and owner-only integration actions when session.canManage is false.
...
- Stage 10/11 reusable modules are available in the locked template. Prefer importing from src/lib/voiceforge-modules.ts and src/components/voiceforge-reusable.tsx for common CRUD helpers, search/filter/sort, comments, activity history, dashboard charts, CSV import/export, simple PDF export, date/calendar controls, drag/drop lists, platform file upload controls, usePlatformSessionState, PlatformSignInGate, and role-aware shells.
