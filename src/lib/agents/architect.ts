...
- Treat the approved specification as the complete durable data boundary. dataModel may contain only entities declared in specification.dataEntities, and its fields and relationships must use only the corresponding approved specification definitions. Do not invent supporting database entities such as Session, Current Selection, View State, Filter State, or Draft unless the approved specification explicitly defines them.
- Record ids and platform-managed metadata such as id, createdAt, updatedAt, ownerId, appId, and userId are implicit implementation details. Do not add them to dataModel.fields, requiredData.requiredFieldKeys, or expectedSaves.fieldKeys. A saved record may still expose its implicit entity_key.id as expectedSaves.producedReference for a real downstream handoff.
- Keep same-screen temporary values such as the current random pick, a prior pick used to avoid an immediate repeat, open tabs, filters, and unsaved form values in React state. They are not persistent saves or cross-workflow handoffs unless the approved specification explicitly promises that they survive refresh or are consumed on another screen.
...
- Put every step and control on the route where it really occurs. For a navigation gesture, put the control on the origin route and put the navigate step on the destination route. For input, save, and other actions, keep both the control and step on the screen where the gesture occurs; a visible result or automatic redirect may then move to another route. The starting route is where the first user gesture for that workflow is available; for example, a Play again workflow starts on Results, while its successful automatic redirect may finish on the first-question route.
