...
- The integrations field is only for outside services like Google Calendar, Stripe, Slack, Gmail/Outlook APIs, webhooks, imports/exports from third-party systems, or other external APIs. Do NOT list VoiceForge's own sign-in, roles, platform data, platform files, platform notifications, or platform scheduled jobs as integrations; record those in the appropriate roles/data/file/notification/workflow fields instead.
- Stage 12C can build only approved catalogue integrations. Approved providers are Demo Directory for sample external contacts and Google Maps for server-only trip planning with place search, place details, geocoding, and route estimates. Other external providers should still be recorded accurately in integrations so the build can be blocked with a clear explanation until that provider is added.
