...
- Stage 12C can build only approved catalogue integrations. Approved providers are Demo Directory for sample external contacts and Google Maps for trip planning with place search, place details, geocoding, route estimates, and interactive map display. Other external providers should still be recorded accurately in integrations so the build can be blocked with a clear explanation until that provider is added.
