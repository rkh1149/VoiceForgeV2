...
Current progress: **Stage 9C — platform data members and roles.** Owners can manage app-level platform data access from the app dashboard by adding existing VoiceForge V2 users as owner, editor, or viewer, changing their role, and removing access. Generated shared apps can now request a VoiceForge member session, show signed-out/no-access/current-role states, and enforce owner/editor/viewer permissions through the locked platform data service. No-sign-in shared-link apps still work for open collaboration, while public apps expose anonymous read-only access.
