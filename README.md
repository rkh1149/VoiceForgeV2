...
Current progress: **Stage 11B — platform notifications and scheduled jobs.** Generated apps can use locked in-app/email notification APIs, user preferences, notification inboxes, and platform-managed scheduled notification job metadata. VoiceForge records preview-ready/build-failed notifications, enforces approved templates/recipient groups/rate limits/quotas, and keeps generated apps away from arbitrary email providers, background workers, and generated API routes. Requires `npm run db:push` (new notification preference, outbox, scheduled job, and job run tables).
