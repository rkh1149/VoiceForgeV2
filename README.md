...
Current progress: **Stage 14F — repair from workflow failure.** Workflow-linked review and browser failures now become structured repair packages that carry the original promise, exact workflow/save/handoff IDs, roles, routes, controls, entities and field keys, producer-consumer graph, failure evidence, and prior attempts. VoiceForge deterministically distinguishes application defects, generated-test defects, and external-service failures; it limits AI edits to the affected workflow graph, retains and checkpoints measurable partial progress within that graph, runs focused static/unit/Playwright validation before accepting a completed candidate, protects unrelated files, and restores an explicit durable pre-repair checkpoint when validation regresses. Build pages show the classification, target surface, validation state, changed application/test files, and protected-file count. Stage 14E provenance still requires `npm run db:push` for installations that have not applied its nullable requirement-to-conversation link.

Stage 11A — platform files and real PDF exports. Generated apps can use locked platform file upload/download/delete APIs with file type, size, count, storage quota, role, and record-link checks. The template includes upload helpers and local fallbacks for tests, and code generation now requires real PDF bytes through approved PDF helpers/packages instead of fake text blobs.

Stage 10 — curated dependencies and reusable modules. Generated apps now start from a locked package catalogue with exact versions for approved UI, forms, charts, tables, date/calendar, drag/drop, Radix primitives, CSV, and PDF packages. The architecture planner selects dependency profiles, code agents may import only approved packages, the build gauntlet includes a dependency/security check, and the generated template includes reusable helpers/components for CRUD-style filtering/sorting, comments, activity history, dashboards, CSV export, calendars, role-aware shells, tables, and sortable lists.

Stage 9C — platform data members and roles. Owners can manage app-level platform data access from the app dashboard by adding existing VoiceForge V2 users as owner, editor, or viewer, changing their role, and removing access. Generated shared apps can request a VoiceForge member session, show signed-out/no-access/current-role states, and enforce owner/editor/viewer permissions through the locked platform data service. No-sign-in shared-link apps still work for open collaboration, while public apps expose anonymous read-only access.
...
Stage 8B — solution architecture gate. Approved specs now get a persisted architecture plan before code generation, including implementation tier, complexity score, pages/components, data model, service needs, file plan, UX plan, tests, risks, and capability validation. Builds that need unsupported generated-app platform services stop with `needs_input` instead of producing a misleading local-only app. Personal/browser-only apps continue into code generation with the architecture plan passed to the code agent. Requires `npm run db:push` (new `architecture_plans` table).
