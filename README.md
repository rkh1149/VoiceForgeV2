...
Current progress: **Stage 10 — curated dependencies and reusable modules.** Generated apps now start from a locked package catalogue with exact versions for approved UI, forms, charts, tables, date/calendar, drag/drop, Radix primitives, CSV, and PDF packages. The architecture planner selects dependency profiles, code agents may import only approved packages, the build gauntlet includes a dependency/security check, and the generated template includes reusable helpers/components for CRUD-style filtering/sorting, comments, activity history, dashboards, CSV export, calendars, role-aware shells, tables, and sortable lists.
...
Stage 8B — solution architecture gate. Approved specs now get a persisted architecture plan before code generation, including implementation tier, complexity score, pages/components, data model, service needs, file plan, UX plan, tests, risks, and capability validation. Builds that need unsupported generated-app platform services (shared server data, generated-app users/roles, files, email, jobs, or external integrations) stop with `needs_input` instead of producing a misleading local-only app. Personal/browser-only apps continue into code generation with the architecture plan passed to the code agent. Requires `npm run db:push` (new `architecture_plans` table).
